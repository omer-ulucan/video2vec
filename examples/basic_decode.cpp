...
namespace {
const char* media_type_name(video2vec::ffmpeg::MediaType type) {
    using video2vec::ffmpeg::MediaType;
    switch (type) {
...
int main(int argc, char** argv) {
    ...
    std::cout << "File: " << demuxer.path() << " (" << demuxer.duration_ms() << " ms)\n";
    auto streams = demuxer.streams();
    for (const auto& s : streams) {
        std::cout << "Stream " << s.index << ": " << s.codec_name
                  << " type=" << media_type_name(s.type)
